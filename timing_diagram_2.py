...
def simulate_shift_register(
    clock, periods, steps_per_period, register_type, use_complement
):
    ...
    for i in range(1, len(clock)):
        if clock[i - 1] == 0 and clock[i] == 1:  # Rising edge detected
            ...
            for j in range(
                i, len(clock)
            ):  # Update from the current clock step to the end
...
def get_user_inputs():
    ...
    print("\nRegister Types:")
    print(
        "1. SISO (Serial-In Serial-Out): Data is shifted in serially, and output is read serially."
    )
    print(
        "2. SIPO (Serial-In Parallel-Out): Data is shifted in serially, but all outputs are updated in parallel."
    )
    print(
        "3. PISO (Parallel-In Serial-Out): Data is loaded into the register in parallel but is output serially."
    )
    print(
        "4. PIPO (Parallel-In Parallel-Out): Data is both loaded and read out in parallel."
    )

    # Get the type of register from the user
    register_type = input(
        "Enter the type of shift register (SISO, SIPO, PISO, PIPO): "
    ).upper()
...
periods, steps_per_period, duty_cycle, clock_start, register_type, use_complement = (
    get_user_inputs()
)
...
Q0, Q1, Q2, Q3 = simulate_shift_register(
    clock, periods, steps_per_period, register_type, use_complement
)
