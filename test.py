...
def simulate_toggle_register(clock, periods, steps_per_period):
    ...
    for i in range(1, len(clock)):
        if clock[i-1] == 0 and clock[i] == 1:  # Rising edge detected
            ...
            Q1[i:] = 1 - Q1[i-1] if Q0[i] == 0 else Q1[i-1]  # Q1 toggles every time Q0 goes low
            Q2[i:] = 1 - Q2[i-1] if Q1[i] == 0 else Q2[i-1]  # Q2 toggles every time Q1 goes low
            Q3[i:] = 1 - Q3[i-1] if Q2[i] == 0 else Q3[i-1]  # Q3 toggles every time Q2 goes low
