    ...
    for i in range(1, len(clock)):
        if clock[i-1] == 0 and clock[i] == 1:  # Rising edge detected
            ...
            for j in range(i, len(clock)):  # Update from the current clock step to the end
                Q0[j] = 1 - Q0[i-1]  # Q0 toggles on every clock edge
                Q1[j] = 1 - Q1[i-1] if Q0[i] == 0 else Q1[i-1]  # Q1 toggles when Q0 goes low
                Q2[j] = 1 - Q2[i-1] if Q1[i] == 0 else Q2[i-1]  # Q2 toggles when Q1 goes low
                Q3[j] = 1 - Q3[i-1] if Q2[i] == 0 else Q3[i-1]  # Q3 toggles when Q2 goes low
